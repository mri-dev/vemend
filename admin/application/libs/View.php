<?	class View {
		...
		function addStyle($style, $after = '', $source = true){
			if($source){
				return '<link rel="stylesheet" type="text/css" href="' . STYLE . $style .'.css'.( (DEVMODE === true ) ? '?t='.uniqid():'' ).'" '.$after.'/>'."\n\r";
			}else{
				return '<link rel="stylesheet" type="text/css" href="' . SSTYLE . $style .'.css'.( (DEVMODE === true ) ? '?t='.uniqid():'' ).'" '.$after.'/>'."\n\r";
			}
		}
		...
		function addJS($file, $type = false, $source = true){
			if($source){
				...
				return '<script type="text/javascript" src="'.$wt.''.( (DEVMODE === true ) ? '?t='.uniqid():'' ).'"></script>'."\n\r";
			}else{
