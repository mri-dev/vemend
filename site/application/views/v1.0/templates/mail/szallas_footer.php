</div>
</div>
</div>
</div>

<footer>
<div class="wb">
 <div class="width pad">
     <table width="100%" border="0">
         <tr>
             <tbody>
                 <tr>
                     <td style="text-align: center; font-size: 10px; color: #333333;">
                        Ezt a levelet a(z) <?=$settings['page_author']?> szállás rendszere küldte automatikusan azzal a céllal, hogy értesítse Önt. Az Ön adatait bizalmasan kezeljük.
                     </td>
                 </tr>
             </tbody>
         </tr>
     </table>
 </div>
</div>
<div class="cdiv"></div>

<div class="width pad footer">
 <div class="row"><strong style="font-size:20px;"><?=$settings['page_author']?></strong><br> &mdash; Szállásajánló rendszere &mdash;</div>
  <div class="row">
      <table style="width: 100%">
          <tbody>
              <tr>
                  <td style="width:33.333%;"><?=$settings['page_author_address']?></td>
                  <td style="width:33.333%;">Telefon: <?=$settings['page_author_phone']?></td>
                  <td style="width:33.333%;">Email: <a href="mailto:<?=$settings['primary_email']?>"><?=$settings['primary_email']?></a></td>
              </tr>
          </tbody>
      </table>
  </div>
  <div class="row">
      <a href="<?=$settings['blog_url']?>"><?=$settings['blog_url']?></a>
  </div>
</div>
</footer>

</body>
</html>
